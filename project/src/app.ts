...
// = HTMLDivElement (default value).
// 제네릭에 T 아무것도 들어오지 않게되면 default value 인 HTMLDivElement 담김.
...
// DOM
// Element  ->  HTMLElement ->  HTMLParagraphElement === p태그
// 제네릭  -----------------------------------------
...
function setDeathsList(data: CountrySummaryResponse[]) {
  const sorted = data.sort(
    (a: CountrySummaryResponse, b: CountrySummaryResponse) => getUnixTimestamp(b.Date) - getUnixTimestamp(a.Date),
  );
  sorted.forEach((value: CountrySummaryResponse) => {
    ...
    li.appendChild(span);
    li.appendChild(p);
    // ! === 타입단언 난 null 이 아니다!!!! 확신이있을 경우 사용하자.
...
function setRecoveredList(data: CountrySummaryResponse[]) {
  ...
  sorted.forEach((value: CountrySummaryResponse) => {
    ...
    li.appendChild(span);
    li.appendChild(p);
    // if(recoveredList === undefined || recoveredList === null) {
    //   return;
    // }
    // else {
    //   recoveredList.appendChild(li);
    // }
    // 옵셔널 체이닝 ? <- 값이 있으면 넣고 없으면 바로 return false ㅇㅇ
    recoveredList?.appendChild(li);
  });
}
...
function setChartData(data: CountrySummaryResponse[]) {
  const chartData = data.slice(-14).map((value: CountrySummaryResponse) => value.Cases);
  const chartLabel = data
    .slice(-14)
    .map((value: CountrySummaryResponse) => new Date(value.Date).toLocaleDateString().slice(5, -1));
  renderChart(chartData, chartLabel);
}

function setTotalConfirmedNumber(data: CovidSummaryResponse) {
  // reduce는  콜백함수의 return값(total)을 누적
  // reduce 함수는 반드시 return 값을 작성해야한다.
